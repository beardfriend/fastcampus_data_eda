...
![](assets/rebuy.png)
![](assets/coupon.png)
`두 번 이상 구매한 고객들은 쿠폰을 사용하지 않는 인원이 더 많았다. 가격보다도 다른 곳에 재구매율의 원인이 있을거라는 단서를 제공해줬다.`

![](assets/midnight.png)
`새벽과 주말에는 환불이 안되는 것으로 보아, 환불 시스템이 자동으로 이뤄지지 않고 있음을 확인했다.`
...
독특하게도 **일러스트 분야에서만 평균적인 추세와 반대되는 흐름**을 확인할 수 있었습니다.
...
MOU를 통해 최소한 단기적으로는  
매출에 긍정적인 영향을 줬음을 확인할 수 있었습니다.

<br/>
<br/>

위에서 도출한
`두 번 이상 구매한 고객들은 쿠폰을 사용하지 않는 인원이 더 많았다. 가격보다도 다른 곳에 재구매율의 원인이 있을거라는 단서를 제공해줬다.` 인사이트를 기반으로

강의의 품질이나, 좋은 여론, 매끄러운 운영 등 다양한 요인이  
재구매에 영향을 끼칠 거라고 판단했습니다.

공부의 스펙트럼이 넓고, 수요가 많은
프로그래밍 카테고리에서
재구매율을 높혔을 때  
매출에 도움을 줄 수 있을거라 판단했고
...
"패스트캠퍼스" 키워드로 okky, dc인사이드에서 데이터 수집을 하였습니다.
...
프로그래밍 카테고리의 부정적인 댓글에 비해
일러스트 쪽은 긍정적인 댓글이 압도적으로 높은 것을 확인할 수 있었습니다.
...
또한 쿠폰을 사용하여 저렴하게 구매할 때 긍정적인 반응이 있는 경우가 종종 있었습니다.
